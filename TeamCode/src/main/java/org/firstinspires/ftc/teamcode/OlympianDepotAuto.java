...
@Disabled
@Autonomous (name = "Olympian Depot Auto", group = "Autonomous")
public class OlympianDepotAuto extends OpMode {
    ...
    public void loop() {
       ...
       /* if (runtime.time() >= 0) {
        ...
        //lowers robot
        if (runtime.time() <= 1.54) {
            lifterMotor.setPower(.5);
        }

        if (runtime.time() >= 1.55) {
        ...
        //moves robot forward
        if (runtime.time() >= 3 && runtime.time() <= 4) {
            move(1, 1, 1, 1);
        }

        //stops robot
        if (runtime.time() >= 4.5 && runtime.time() <= 7) {
            move(0, 0, 0, 0);
            markerServo.setPosition(1);
        }
        //drops marker
        if (runtime.time() >= 7 && runtime.time() <= 8){
            markerServo.setPosition(.5);
        }
        // backs up robot
        if (runtime.time() >= 8.25 && runtime.time() <= 8.5){
            move(-1,-1,-1,-1);
        }
        //turns robot
        if (runtime.time() >= 8.5 && runtime.time() <= 8.6){
            move(1,1,-1,-1);
        }
        if (runtime.time() >= 8.76 && runtime.time() <= 9.8){
            move(-1,1,1,-1);
        }
        // backs robot into the crater
        if (runtime.time() >= 9.9 && runtime.time() <= 12.2){
            move(-1,-1,-1,-1);
        }
        // stops robot
        if (runtime.time() >= 12.3){
            move(0,0,0,0);
        }
    }
}